...
async def create_research(
    ...
    logger.info("Research request received: title=%r, source=%s, request_id=%s",body.eventTitle,body.eventSource,request_id)

    try:

        api_key = "123" # Hard coded for now
        result = await run_research_pipeline(body, api_key, request_id)
        logger.info("Research response sent: cached=%s, request_id=%s",result["cached"],request_id)
